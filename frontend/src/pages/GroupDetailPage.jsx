    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-6 lg:grid-cols-[320px_1fr]">
                ...
                <div key={member.id} className="flex items-center justify-between gap-2 py-3 text-sm">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className={member.leave_date ? 'badge text-slate-500' : 'badge border-teal-200 bg-teal-50 text-accent'}>
          ...
          <div className="panel p-4">
            ...
            <div className="mt-3 space-y-2">
              {balances?.summary.map((row) => (
                <div key={row.userId} className="flex justify-between rounded-md bg-slate-50 px-3 py-2 text-sm">
                  <span>{row.name}</span>
        ...
        <section className="panel overflow-hidden">
          <div className="flex flex-wrap gap-2 border-b border-line bg-slate-50 p-3">
            {['expenses', 'settlements', 'balances', 'import'].map((tab) => (
              <button
                key={tab}
                className={activeTab === tab ? 'bg-ink px-3 py-2 text-white shadow-sm' : 'bg-white px-3 py-2 text-slate-700 hover:bg-slate-100'}
          ...
          {activeTab === 'import' && (
            <div className="p-4">
              {!report && (
                <label className="block cursor-pointer rounded-lg border border-dashed border-line bg-slate-50 p-8 text-center transition hover:border-accent hover:bg-teal-50/40">
